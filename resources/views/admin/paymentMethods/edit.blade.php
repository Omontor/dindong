...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payment-methods.update", [$paymentMethod->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="code">{{ trans('cruds.paymentMethod.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $paymentMethod->code) }}" required>
                @if($errors->has('code'))
                    <span class="text-danger">{{ $errors->first('code') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.paymentMethod.fields.code_helper') }}</span>
            </div>
            <div class="form-group">
