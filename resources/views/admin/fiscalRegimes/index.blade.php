...
@can('fiscal_regime_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.fiscal-regimes.create') }}">
                        ...
                        <tr data-entry-id="{{ $fiscalRegime->id }}">
                            ...
                            <td>
                                @can('fiscal_regime_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.fiscal-regimes.show', $fiscalRegime->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('fiscal_regime_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.fiscal-regimes.edit', $fiscalRegime->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('fiscal_regime_delete')
                                    <form action="{{ route('admin.fiscal-regimes.destroy', $fiscalRegime->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('fiscal_regime_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.fiscal-regimes.massDestroy') }}",
