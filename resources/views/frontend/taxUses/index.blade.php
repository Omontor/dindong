        ...
        <div class="col-md-12">
            @can('tax_use_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.tax-uses.create') }}">
                            ...
                            <tbody>
                                @foreach($taxUses as $key => $taxUse)
                                    <tr data-entry-id="{{ $taxUse->id }}">
                                        ...
                                        <td>
                                            @can('tax_use_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.tax-uses.show', $taxUse->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('tax_use_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.tax-uses.edit', $taxUse->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('tax_use_delete')
                                                <form action="{{ route('frontend.tax-uses.destroy', $taxUse->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('tax_use_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.tax-uses.massDestroy') }}",
