        ...
        <div class="col-md-12">
            @can('product_unit_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.product-units.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $productUnit->id }}">
                                        ...
                                        <td>
                                            @can('product_unit_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-units.show', $productUnit->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_unit_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-units.edit', $productUnit->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_unit_delete')
                                                <form action="{{ route('frontend.product-units.destroy', $productUnit->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_unit_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.product-units.massDestroy') }}",
