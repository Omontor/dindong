        ...
        <div class="col-md-12">
            @can('municipality_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.municipalities.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $municipality->id }}">
                                        ...
                                        <td>
                                            @can('municipality_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.municipalities.show', $municipality->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('municipality_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.municipalities.edit', $municipality->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('municipality_delete')
                                                <form action="{{ route('frontend.municipalities.destroy', $municipality->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('municipality_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.municipalities.massDestroy') }}",
